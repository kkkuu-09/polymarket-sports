...
# 插入数据（表名改为nba_market_price）
def insert_price_batch(rows):
    db = get_db()
    cursor = db.cursor()
    sql = """
    INSERT INTO nba_market_price (
...
def main():
    ...
    print(f"NBA赛事数量: {len(events)}")

    print("筛选NBA胜负盘...")
    markets = get_moneyline_markets(events)
    print(f"NBA胜负盘数量: {len(markets)}")

    print("开始抓取NBA分钟级价格并入库...")
    ...
    print("\n✅ NBA数据全部完成！可在Navicat查看nba库的nba_market_price表")
