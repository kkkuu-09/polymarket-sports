...
# ========== 查询函数（表名：mlb_market_price） ==========
@st.cache_data(ttl=30)
def get_total(sd, ed, ev, t1, t2, p_choose, pmi, pma):
    base = """
    SELECT COUNT(*) AS cnt FROM mlb_market_price
...
if not df_all.empty:
    ...
    download_placeholder.download_button(
        label="📥 下载全部查询数据",
        data=csv_data,
        file_name=f"MLB赛事数据_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
        mime="text/csv",
...
# ========== 赔率走势折线图（1分钟级） ==========
if not df_all.empty:
    st.subheader("📈 MLB 主客队 1 分钟级赔率走势")
    dfc = df_all.copy()
